...
public class Java8CookBook {
    ...
    public Java8CookBook(int year, String nationality, String name, String team, int lengthKm, Duration winningTime, int daysInYellow) {
        this.year = year;
    ...
    public static final List<Java8CookBook> tdfWinners = Arrays.asList(
            new Java8CookBook(2006, "Spain", "Óscar Pereiro", "Caisse d'Epargne–Illes Balears", 3657, Duration.parse("PT89H40M27S"), 8),
            new Java8CookBook(2007, "Spain", "Alberto Contador", "Discovery Channel", 3570, Duration.parse("PT91H00M26S"), 4),
            new Java8CookBook(2008, "Spain", "Carlos Sastre", "Team CSC", 3559, Duration.parse("PT87H52M52S"), 5),
            new Java8CookBook(2009, "Spain", "Alberto Contador", "Astana", 3459, Duration.parse("PT85H48M35S"), 7),
            new Java8CookBook(2010, "Luxembourg", "Andy Schleck", "Team Saxo Bank", 3642, Duration.parse("PT91H59M27S"), 12),
            new Java8CookBook(2011, "Australia", "Cadel Evans", "BMC Racing Team", 3430, Duration.parse("PT86H12M22S"), 2),
            new Java8CookBook(2012, "Great Britain", "Bradley Wiggins", "Team Sky", 3496, Duration.parse("PT87H34M47S"), 14),
            new Java8CookBook(2013, "Great Britain", "Chris Froome", "Team Sky", 3404, Duration.parse("PT83H56M20S"), 14),
            new Java8CookBook(2014, "Italy", "Vincenzo Nibali", "Astana", 3661, Duration.parse("PT89H59M06S"), 19),
            new Java8CookBook(2015, "Great Britain", "Chris Froome", "Team Sky", 3360, Duration.parse("PT84H46M14S"), 16),
            new Java8CookBook(2016, "Great Britain", "Chris Froome", "Team Sky", 3529, Duration.parse("PT89H04M48S"), 14));
    ...
    public static void main(String args[]) {
        ...
        List<String> winnersOfToursLessThan3500km = tdfWinners
                ...
                .map(Java8CookBook::getName) // Get names of winners
        ...
        // limit -
        List<Java8CookBook> winnerObjectsOfToursLessThan3500kmLimit2 = tdfWinners
                .stream()
                .filter(d -> d.getLengthKm() < 3500)
                .limit(2)
                .collect(toList());
        ...
        // skip records
        List<Java8CookBook> skipEveryOtherTDFWinner = tdfWinners
                .stream()
                .skip(2)
                .collect(toList());
        ...
        Optional<Java8CookBook> winner2012 = tdfWinners.stream().filter(w -> w.getName().contains("Wiggins")).findAny();
        // winner2012 - Bradley Wiggins
        System.out.println("winner2012 - " + winner2012.get());
        Optional<Integer> winnerYear2014 = tdfWinners.stream().map(Java8CookBook::getYear).filter(x -> x == 2014).findFirst();
        // winnerYear2014 - 2014
        System.out.println("winnerYear2014 - " + winnerYear2014.get());
        // reducing - 0 --> initial value
        int totalDistance = tdfWinners.stream().map(Java8CookBook::getLengthKm).reduce(0, Integer::sum);
        // totalDistance - 38767
        System.out.println("totalDistance - " + totalDistance);
        Optional<Integer> shortestYear = tdfWinners.stream().map(Java8CookBook::getLengthKm).reduce(Integer::min);
        // shortestYear - 3360
        System.out.println("shortestYear - " + shortestYear.get());
        Optional<Integer> longestYear = tdfWinners.stream().map(Java8CookBook::getLengthKm).reduce(Integer::max);
        // longestYear - 3661
        System.out.println("longestYear - " + longestYear.get());
        Optional<Java8CookBook> fastestWinner = tdfWinners.stream().min(Comparator.comparingDouble(Java8CookBook::getAveSpeed));
        System.out.println("fastestTDF - " + fastestWinner.get());
        // shorthand
        OptionalDouble fastestTDF = tdfWinners.stream().mapToDouble(Java8CookBook::getAveSpeed).min();
        // fastestTDF - 39.0
        System.out.println("fastestTDF - " + fastestTDF.getAsDouble());
        // groupingby - make a map whose keys are names
        Map<String, List<Java8CookBook>> namesVsWinner = tdfWinners.stream().collect(groupingBy(Java8CookBook::getName));
        ...
        // join strings
        String allTDFWinnersTeamsCSV = tdfWinners.stream().map(Java8CookBook::getTeam).collect(joining(", "));
        ...
        // grouping
        Map<String, List<Java8CookBook>> winnersByNationality = tdfWinners.stream().collect(groupingBy(Java8CookBook::getNationality));
        ...
        Map<String, Long> winsByNationalityCounting = tdfWinners.stream().collect(groupingBy(Java8CookBook::getNationality, counting()));
